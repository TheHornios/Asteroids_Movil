...
public class DisparoPor3 extends Mejoras{
	public DisparoPor3(Juego1 juego) {
		super(juego);
		setTextura(Assets.mejora_disparoX2);
	...
	public void draw(Canvas g) {
		...
		g.drawBitmap(textura,null,e,g2);
		if(box.intersect(nave.getBox())){
			juego.crearExplosion(new Vector2D(posicionMejora.getX(),posicionMejora.getY()));
			if(juego.getNave().idTriple()){
				juego.getNave().setTriple2(true);
			}

			juego.getNave().setTriple(true);
			juego.getOMensajes().add(new Mensaje(new Vector2D(posicionMejora.getX(),posicionMejora.getY()), true, "Disparo X 3", juego,60));
